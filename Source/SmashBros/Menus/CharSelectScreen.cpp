...
namespace SmashBros
{
	...
	{// settings for character select ,,position down,,,spacing +
		CharIcon*a = new CharIcon(num, 0,0, new Animation("normal",1,anim));
		addToGrid(a,70,350,20,5,240,65,charIcons.size()+2);
		if(charIcons.size()==0)
		{
	...
	{
        //sets character selection icon size
		setScale(3.8f);
		addAnimation(anim);
