...
namespace SmashBros
{
	void Charizard::addProjectileInfo(byte type, int projID, float x, float y)
	{
		ProjectileInfo info;
		info.type = type;
		info.projID = projID;
	...
	Charizard::Charizard(float x1, float y1, byte playerNo, byte team) : Player(x1, y1, playerNo, team)
	{
		charging_attack = 0;
	...
	void Charizard::setToDefaultValues()
	{
		charging_attack = 0;
	}
	...
	void Charizard::Load()
	{
		setFolderPath("Images/Game/Characters/Charizard/");
		
		loadFile("Images/Game/Characters/Charizard/fireball.png");
		loadFile("Images/Game/Characters/Charizard/fludd_water.png");
		loadFile("Images/Game/Characters/Charizard/finalsmash_fire.png");
		addTwoSidedAnimation("stand", "stand.png", 10, 1, 13);
	...
	void Charizard::LoadAttackTypes()
	{
	...
	void Charizard::Unload()
	{
		Player::Unload();
	}
	
	void Charizard::addP2PData(DataVoid&data)
	{
	...
	void Charizard::setP2PData(byte*&data)
	{
	...
	void Charizard::Update(long gameTime)
	{
	...
	void Charizard::Draw(Graphics2D&g, long gameTime)
	{
		Player::Draw(g, gameTime);
	}
	
	void Charizard::onAnimationFinish(const String&n)
	{
	...
	void Charizard::onPlayerHit(Player*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Charizard::onPlayerRectHit(Player*collide, byte dir)
	{
		if(attacksHolder==12)
		{
			switch(hitAmount)
	...
	boolean Charizard::onDeflectProjectileCollision(Projectile*collide, byte dir)
	{
	...
	boolean Charizard::onDeflectItemCollision(Item*collide, byte dir)
	{
		byte relDir = getRelPlayerDir(collide);
		if(attacksHolder==11 && (relDir==getPlayerDir() || relDir==0))
		{
	...
	void Charizard::doChargingAttack(byte button)
	{
		destroyCharge();
	...
	void Charizard::attackSideSmash(byte type)
	{
	...
	void Charizard::attackUpSmash(byte type)
	{
	...
	void Charizard::attackDownSmash(byte type)
	{
	...
	void Charizard::attackFinalSmash()
	{
	...
	Charizard::Fireball::Fireball(byte playerNo, float x1, float y1) : Projectile(playerNo,x1,y1)
	{
		xspeed = 3.1f;
		yspeed = 2;
		...
		anim->addFrame("Images/Game/Characters/Charizard/fireball.png");
		addAnimation(anim);
	...
	Charizard::Fireball::~Fireball()
	{
		//
	}
	
	void Charizard::Fireball::deflect(byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Charizard::Fireball::Update(long gameTime)
	{
	...
	void Charizard::Fireball::onPlayerHit(Player*collide, byte dir)
	{
	...
	void Charizard::Fireball::whilePlatformColliding(Platform*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_UP:
	...
	Charizard::FluddWater::FluddWater(byte playerNo, float x1, float y1) : Projectile(playerNo, x1, y1)
	{
		Animation*anim;
			
		anim = new Animation("left",10,10,1);
		anim->addFrame("Images/Game/Characters/Charizard/fludd_water.png");
		addAnimation(anim);
	...
	Charizard::FluddWater::~FluddWater()
	{
		//
	}

	void Charizard::FluddWater::Update(long gameTime)
	{
	...
	void Charizard::FluddWater::whilePlayerHitting(Player*collide, byte dir)
	{
	...
	void Charizard::FluddWater::onAnimationFinish(const String&aName)
	{
		destroy();
		Charizard*playr = (Charizard*)Global::getPlayerActor(getPlayerNo());
		playr->animFinish();
	}
	
	Charizard::FinalSmashFire::FinalSmashFire(byte playerNo, float x1, float y1) : Projectile(playerNo,x1,y1)
	{
		Animation*anim;
		
		anim = new Animation("left", 12, 5, 2);
		anim->addFrame("Images/Game/Characters/Charizard/finalsmash_fire.png");
		addAnimation(anim);
	...
	Charizard::FinalSmashFire::~FinalSmashFire()
	{
		//
	}
		
	void Charizard::FinalSmashFire::onPlayerHit(Player*collide, byte dir)
	{
		if(isHittable(collide,dir))
		{
			switch(dir)
	...
	void Charizard::FinalSmashFire::onAnimationFinish(const String&n)
	{
		Player*playr = Global::getPlayerActor(this->getPlayerNo());
