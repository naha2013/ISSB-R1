...
namespace SmashBros
{
	void Dante::addProjectileInfo(byte type, int projID, float x, float y)
	{
		ProjectileInfo info;
		info.type = type;
		info.projID = projID;
	...
	Dante::Dante(float x1, float y1, byte playerNo, byte team) : Player(x1, y1, playerNo, team)
	{
		charging = 0;
		finalSmash = false;
	...
	void Dante::addP2PData(DataVoid&data)
	{
	...
	void Dante::setP2PData(byte*&data)
	{
	...
	void Dante::Load()
	{
		setFolderPath("Images/Game/Characters/Dante/");
		
		loadFile("Images/Game/Characters/Dante/projectile.png");
		loadFile("Images/Game/Characters/Dante/down_blast.png");
		loadFile("Images/Game/Characters/Dante/finalsmash_blast.png");
		loadFile("Images/Game/Characters/Dante/blade.png");
	...
	void Dante::onAnimationFinish(const String&n)
	{
	...
	void Dante::Update(long gameTime)
	{
	...
	void Dante::Draw(Graphics2D&g, long gameTime)
	{
	...
	void Dante::whilePlayerColliding(Player*collide, byte dir)
	{
		if(attacksHolder==11)
		{
	...
	void Dante::onPlayerHit(Player*collide, byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Dante::onFinishCharge()
	{
		//
	}
	
	void Dante::doChargingAttack(byte button)
	{
		if(charging==1)
		{
			destroyCharge();
	...
	void Dante::attackSideSmash(byte type)
	{
	...
	void Dante::attackUpSmash(byte type)
	{
	...
	void Dante::attackDownSmash(byte type)
	{
	...
	Dante::Blast::Blast(byte playerNo, float x1, float y1) : Projectile(playerNo, x1, y1)
	{
		Animation*anim;
		anim = new Animation("left",1,"Images/Game/Characters/Dante/projectile.png");
		addAnimation(anim);
			
		anim = new Animation("right",1,"Images/Game/Characters/Dante/projectile.png");
		anim->mirror(true);
		addAnimation(anim);
	...
	void Dante::Blast::deflect(byte dir)
	{
		switch(dir)
		{
			case DIR_LEFT:
	...
	void Dante::Blast::Update(long gameTime)
	{
	...
	void Dante::Blast::onPlayerHit(Player*collide, byte dir)
	{
	...
	Dante::SpinningBlade::SpinningBlade(byte playerNo, float x1, float y1) : Projectile(playerNo,x1,y1)
	{
		Animation*anim;
			
		anim = new Animation("left",12,8,1);
		anim->addFrame("Images/Game/Characters/Dante/blade.png");
		addAnimation(anim);
	...
	Dante::SpinningBlade::~SpinningBlade()
	{
		//
	};
	
	void Dante::SpinningBlade::Update(long gameTime)
	{
	...
	void Dante::SpinningBlade::Draw(Graphics2D&g, long gameTime)
	{
		Player*owner = Global::getPlayerActor(getPlayerNo());
	...
	void Dante::SpinningBlade::onAnimationFinish(const String&n)
	{
		Player*owner = Global::getPlayerActor(getPlayerNo());
	...
	void Dante::SpinningBlade::onPlayerHit(Player*collide, byte dir)
	{
		if(isHittable(collide,dir))
		{
	...
	Dante::GroundStab::GroundStab(byte playerNo, float x1, float y1) : Projectile(playerNo, x1,y1)
	{
		Animation*anim;
		anim = new Animation("normal",6,4,1);
		anim->addFrame("Images/Game/Characters/Dante/down_blast.png");
		addAnimation(anim);
	...
	Dante::GroundStab::~GroundStab()
	{
		//
	}
		
	void Dante::GroundStab::onPlayerHit(Player*collide, byte dir)
	{
	...
	void Dante::GroundStab::onAnimationFinish(const String&n)
	{
		if(n.equals("normal"))
		{
			destroy();
	...
	Dante::FinalSmashBlast::FinalSmashBlast(byte playerNo, float x1, float y1) : Projectile(playerNo, x1,y1)
	{
		setScale(1.6f);
		
		addAnimation(new Animation("left",1,"Images/Game/Characters/Dante/finalsmash_blast.png"));
		Animation*anim = new Animation("right",1,"Images/Game/Characters/Dante/finalsmash_blast.png");
		anim->mirror(true);
		addAnimation(anim);
	...
	Dante::FinalSmashBlast::~FinalSmashBlast()
	{
		//
	}
		
	void Dante::FinalSmashBlast::onPlayerHit(Player*collide, byte dir)
	{
		if(isHittable(collide,dir))
		{
			switch(dir)
